...
async def create_forecast_run(
    ...
    current_user: InternalActor = Depends(require_planner_access),
):
...
async def list_forecast_runs(
    ...
    current_user: InternalActor = Depends(require_forecast_access),
):
...
async def create_production_plan(
    ...
    current_user: InternalActor = Depends(require_production_access),
):
...
async def list_production_plans(
    forecast_run_id: Optional[int] = Query(None),
    status_filter: Optional[str] = Query(None, alias="status"),
    limit: int = Query(50, le=100),
    current_user: InternalActor = Depends(require_production_access),
):
